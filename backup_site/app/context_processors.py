...
def load_info(request):
    ...
    extra_context["events_link"] = Event.objects.filter(language=request.LANGUAGE_CODE)
    extra_context["partners_link"] = Partners.objects.filter(language=request.LANGUAGE_CODE)
    extra_context["products_link"] = Products.objects.filter(language=request.LANGUAGE_CODE)
